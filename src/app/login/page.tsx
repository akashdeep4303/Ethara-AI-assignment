...
function LoginForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(messageFromApiError(data, "Unable to sign in", res.status));
        return;
      }
      ...
      setError(`Unable to sign in: ${msg}`);
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    ...
    >
      ...
      >
        <defs>
          <radialGradient id="bg-g1" cx="20%" cy="30%" r="60%">
            <stop offset="0%" stopColor="#1e40af" stopOpacity="0.8" />
            <stop offset="100%" stopColor="#0f172a" stopOpacity="0" />
          </radialGradient>
          <radialGradient id="bg-g2" cx="80%" cy="70%" r="55%">
            <stop offset="0%" stopColor="#6d28d9" stopOpacity="0.6" />
            <stop offset="100%" stopColor="#0f172a" stopOpacity="0" />
          </radialGradient>
          <radialGradient id="bg-g3" cx="60%" cy="10%" r="45%">
        ...
        {/* Rings */}
        <circle cx="180" cy="750" r="280" fill="none" stroke="rgba(99,102,241,0.1)" strokeWidth="1" />
        <circle cx="180" cy="750" r="200" fill="none" stroke="rgba(99,102,241,0.08)" strokeWidth="1" />
        <circle cx="1260" cy="150" r="240" fill="none" stroke="rgba(14,165,233,0.1)" strokeWidth="1" />
        <circle cx="1260" cy="150" r="160" fill="none" stroke="rgba(14,165,233,0.07)" strokeWidth="1" />
        <circle cx="720" cy="450" r="400" fill="none" stroke="rgba(255,255,255,0.025)" strokeWidth="0.5" />
        {/* Stars */}
        <circle cx="320" cy="200" r="2.5" fill="rgba(255,255,255,0.25)" />
        <circle cx="1100" cy="100" r="2" fill="rgba(255,255,255,0.2)" />
        <circle cx="1320" cy="600" r="2.5" fill="rgba(255,255,255,0.15)" />
        <circle cx="100" cy="500" r="2" fill="rgba(255,255,255,0.2)" />
        <circle cx="900" cy="800" r="1.5" fill="rgba(255,255,255,0.18)" />
        <circle cx="600" cy="80" r="2" fill="rgba(255,255,255,0.15)" />
        <circle cx="1180" cy="700" r="1.5" fill="rgba(255,255,255,0.2)" />
        <circle cx="420" cy="820" r="2" fill="rgba(255,255,255,0.12)" />
        <circle cx="50" cy="150" r="1.5" fill="rgba(255,255,255,0.18)" />
        <circle cx="1380" cy="320" r="2" fill="rgba(255,255,255,0.15)" />
        {/* Geometric shapes */}
        <rect x="80" y="80" width="70" height="70" rx="10" fill="none" stroke="rgba(255,255,255,0.04)" strokeWidth="1" transform="rotate(15 115 115)" />
        <rect x="1280" y="720" width="60" height="60" rx="8" fill="none" stroke="rgba(255,255,255,0.04)" strokeWidth="1" transform="rotate(-20 1310 750)" />
        <rect x="1240" y="280" width="50" height="50" rx="6" fill="none" stroke="rgba(99,102,241,0.1)" strokeWidth="1" transform="rotate(30 1265 305)" />
        <line x1="0" y1="0" x2="1440" y2="900" stroke="rgba(255,255,255,0.015)" strokeWidth="1" />
        <line x1="1440" y1="0" x2="0" y2="900" stroke="rgba(255,255,255,0.015)" strokeWidth="1" />
      </svg>
          ...
          >
            ...
            <div className="relative z-10 py-8">
              ...
              <p className="text-[13px] leading-relaxed" style={{ color: "rgba(255,255,255,0.55)" }}>
                Your team&apos;s work, in one place —<br />organized, prioritized, always on track.
              </p>
            </div>
            ...
            <div className="relative z-10 flex flex-col gap-2">
              {[
                { dot: "#34d399", label: "Design new onboarding flow", badge: "Done", bg: "rgba(52,211,153,0.15)", color: "#34d399" },
                { dot: "#7cb9ff", label: "Integrate payment gateway", badge: "In Progress", bg: "rgba(59,126,248,0.22)", color: "#7cb9ff" },
                { dot: "#fabd4b", label: "Q3 performance review", badge: "Review", bg: "rgba(250,189,75,0.15)", color: "#fabd4b" },
              ].map((t) => (
                <div
                  key={t.label}
                  className="flex items-center gap-2.5 rounded-[10px] border border-white/10 px-3 py-2.5"
                  style={{ background: "rgba(255,255,255,0.08)" }}
                >
                  <div className="h-2 w-2 flex-shrink-0 rounded-full" style={{ background: t.dot }} />
                  <span className="flex-1 text-[12px]" style={{ color: "rgba(255,255,255,0.75)" }}>{t.label}</span>
            ...
            <div className="relative z-10 mt-5 flex gap-3">
              {[{ num: "24", lbl: "Tasks today" }, { num: "6", lbl: "Done" }].map((s) => (
                <div
                  key={s.lbl}
                  className="flex items-center gap-1.5 rounded-full border border-white/10 px-3.5 py-1.5"
                  style={{ background: "rgba(255,255,255,0.08)" }}
                >
                  <span className="text-[14px] font-bold text-white">{s.num}</span>
                  <span className="text-[11px]" style={{ color: "rgba(255,255,255,0.45)" }}>{s.lbl}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          >
            ...
            <div className="mb-7">
              ...
              <p className="mt-1.5 text-[13px]" style={{ color: "#8492a6" }}>
                ...
                <Link href="/register" className="font-semibold" style={{ color: "#3B7EF8" }}>
                  Sign up free
                </Link>
              </p>
            </div>
            ...
            <div className="mb-4">
              ...
              >
                ...
                <input
                  ...
                  style={{ flex: 1, border: "none", background: "transparent", padding: "10px 12px 10px 0", fontSize: "13.5px", color: "#0B1540", outline: "none" }}
                />
              </div>
            </div>
            ...
            <div className="mb-3">
              <div className="mb-1.5 flex items-center justify-between">
                <label className="text-[12px] font-semibold uppercase tracking-[0.5px]" style={{ color: "#3D4A5C" }}>Password</label>
                <Link href="/forgot-password" className="text-[12px] font-medium" style={{ color: "#3B7EF8" }}>Forgot password?</Link>
              </div>
              <div
                className="flex items-center overflow-hidden rounded-[10px] transition-all"
                style={{ border: "1.5px solid #E4EAF2", backgroundColor: "#F7FAFF" }}
            ...
            <div className="mb-5 flex items-center gap-2.5">
              <input type="checkbox" id="remember" checked={remember} onChange={(e) => setRemember(e.target.checked)}
                style={{ width: 15, height: 15, accentColor: "#3B7EF8", cursor: "pointer" }} />
              <label htmlFor="remember" className="cursor-pointer select-none text-[13px]" style={{ color: "#8492a6" }}>
            ...
            {/* ── Submit ── */}
            <button
              type="submit" disabled={loading} onClick={onSubmit}
              ...
              style={{ background: "#3B7EF8", boxShadow: "0 4px 18px rgba(59,126,248,0.35)" }}
              onMouseEnter={(e) => { if (!loading) (e.currentTarget as HTMLButtonElement).style.background = "#2563EB"; }}
              onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "#3B7EF8"; }}
            >
              {loading ? (
                <>
            ...
            <div className="my-5 flex items-center gap-3">
              ...
              <span className="text-[11px] font-medium" style={{ color: "#C4CEDB" }}>or continue with</span>
              <div className="h-px flex-1" style={{ background: "#EDF1F7" }} />
            </div>
